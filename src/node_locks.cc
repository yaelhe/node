    ...
    if (IsGrantable(request.get())) {
      ...
      Local<Value> args[] = {
          ...
          Function::New(env->context(),
                        ...
                        External::New(env->isolate(), lock)).ToLocalChecked(),
      };
...
void LockManager::Request(const FunctionCallbackInfo<Value>& args) {
  ...
  Lock::Mode mode = static_cast<Lock::Mode>(args[3].As<Number>()
      ->Int32Value(env->context()).FromJust());  // TODO(yaelhe): from maybe
    ...
    for (uint32_t i = 0; i < manager->pending_requests_.size(); i += 1) {
      ...
      obj->Set(context,
               ...
               String::NewFromUtf8(isolate,
                  ...
                  .ToLocalChecked())  // TODO(yaelhe): fix maybe
          .ToChecked();
      obj->Set(context,
               env->mode_string(),
